...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return (
        <ToastContext.Provider value={{ addToast, removeToast }}>
            ...
            `}
                    >
                        {toast.type === 'success' && <CheckCircle size={20} weight="regular" className="text-green-500" />}
                        {toast.type === 'error' && <WarningCircle size={20} weight="regular" className="text-red-500" />}
                        {toast.type === 'info' && <Info size={20} weight="regular" className="text-blue-500" />}
                        <span className="font-medium text-sm text-slate-900 dark:text-white">{toast.message}</span>
