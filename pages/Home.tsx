...
import { ArrowRight, InstagramLogo, Calendar, Briefcase, Monitor, Trophy, TrendUp, Users, Star, Leaf, MapPin, Phone, Envelope, Tag, X } from 'phosphor-react';
...
const InstagramPost = ({ post, index }: { post: InstagramPostData; index: number }) => {
    ...
    <LiquidBackground className="pb-14">
      ...
      <section className="relative pt-40 pb-16 md:pt-52 md:pb-20 min-h-[85vh] flex flex-col items-center justify-center">
        <div className="container mx-auto px-6 max-w-6xl relative z-10">
          ...
          <div className="flex flex-col items-center text-center max-w-4xl mx-auto mb-24">
            ...
            >
              Comprometida com a formação integral dos alunos e com a construção de um futuro de realizações profissionais e acadêmicas.
            </motion.p>
            ...
            >
              <Link to="/courses">
                <GlassButton size="lg" variant="primary" className="w-full sm:w-auto px-10 py-4 text-lg shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-center">
            {HIGHLIGHTS.map((item, index) => (
              ...
              >
                ...
                <h3 className="font-bold font-display text-xl text-slate-900 dark:text-white mb-2">{item.title}</h3>
                <p className="text-slate-500 dark:text-slate-400 leading-relaxed text-base max-w-xs mx-auto">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="container mx-auto px-6 max-w-6xl py-32 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <GlassCard className="h-full p-8 hover:bg-white/60 dark:hover:bg-slate-900/60 transition-colors">
            ...
            <h3 className="text-2xl font-bold font-display text-slate-900 dark:text-white mb-3">Gestão e Negócios</h3>
          ...
          <GlassCard className="h-full p-8 hover:bg-white/60 dark:hover:bg-slate-900/60 transition-colors">
            ...
            <h3 className="text-2xl font-bold font-display text-slate-900 dark:text-white mb-3">Recursos Naturais</h3>
            <p className="text-slate-600 dark:text-slate-400 mb-8 leading-relaxed text-sm">
          ...
          <GlassCard className="h-full p-8 hover:bg-white/60 dark:hover:bg-slate-900/60 transition-colors">
            ...
            <h3 className="text-2xl font-bold font-display text-slate-900 dark:text-white mb-3">Info & Comunicação</h3>
            <p className="text-slate-600 dark:text-slate-400 mb-8 leading-relaxed text-sm">
      ...
      <section className="container mx-auto px-6 max-w-6xl pt-20 pb-6">
        <div className="flex flex-col md:flex-row items-center justify-between mb-20 gap-6">
           ...
           <a href={`https://www.instagram.com/${INSTAGRAM_PROFILE}/`} target="_blank" rel="noreferrer">
      ...
      <section className="container mx-auto px-6 max-w-6xl py-32">
        <div className="text-center mb-20">
          ...
          <div className="section-divider mb-6"></div>
          <p className="section-subtitle text-center max-w-2xl mx-auto">
            Acompanhe os processos seletivos, eventos culturais e conquistas acadêmicas.
          </p>
        </div>
          ...
          <div className="bg-white dark:bg-slate-950 rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 max-w-4xl w-full overflow-hidden max-h-[90vh] flex flex-col">
            <div className="relative h-64 shrink-0">
              ...
              <div className="absolute bottom-6 left-6 right-6 text-white">
                <h2 className="text-3xl md:text-4xl font-bold font-display leading-tight">{selectedNews.title}</h2>
              </div>
            </div>
            ...
            <div className="p-8 md:p-10 overflow-y-auto custom-scrollbar space-y-8">
              ...
              {((selectedNews.timeline && selectedNews.timeline.length > 0) || (selectedNews.attachments && selectedNews.attachments.length > 0)) && (
                        ...
                        {selectedNews.attachments.map((att, idx) => (
                          ...
                          >
                            ...
                            <div className="flex-grow">
                              <h4 className="font-bold font-display text-sm text-slate-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{att.name}</h4>
                              <span className="text-xs text-slate-500 dark:text-slate-400 uppercase">{att.type}</span>
                            </div>
                          </a>
                        ))}
                      </div>
              ...
              <div className="flex justify-end pt-6">
                <GlassButton variant="ghost" size="md" onClick={() => setSelectedNews(null)}>Fechar</GlassButton>
              </div>
            </div>
          </div>
        </div>
