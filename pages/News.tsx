...
import { Calendar, MagnifyingGlass, Tag, Funnel, X, ArrowRight } from 'phosphor-react';
      ...
      <section className="relative pt-40 pb-24 overflow-hidden flex items-center justify-center min-h-[50vh]">
        <div className="container mx-auto px-6 text-center relative z-10 max-w-4xl">
          <div className="flex flex-col items-center">
            ...
            >
              Notícias & <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-600 to-teal-500">Editais</span>
            </motion.h1>
            <motion.p 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            {categories.map(cat => (
              <button
                ...
                className={cn(
                  ...
                  filter === cat ? 'pill-primary' : 'pill-neutral hover:bg-white/90 dark:hover:bg-slate-900/90'
                )}
              >
                {cat}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredNews.length > 0 ? (
            filteredNews.map(item => (
              <GlassCard key={item.id} className="flex flex-col h-full hover:bg-white/60 dark:hover:bg-slate-900/60 transition-colors group cursor-pointer" onClick={() => setSelectedNews(item)}>
