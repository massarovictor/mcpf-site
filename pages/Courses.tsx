...
export const Courses: React.FC = () => {
  ...
  const fallbackImage =
    "https://images.unsplash.com/photo-1500336624523-d727130c3328?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80";
      ...
      <div className="container mx-auto px-6 max-w-6xl py-20">
        ...
        <GlassCard className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12 sticky top-24 z-30 p-4 rounded-full">
          ...
          <div className="relative w-full md:w-80">
            <MagnifyingGlass
              className="absolute left-4 top-1/2 -translate-y-1/2 text-primary-700 dark:text-primary-300 drop-shadow-sm pointer-events-none transition-colors z-10"
              size={20}
              weight="bold"
            />
            <input
          ...
          <div className="bg-white dark:bg-slate-950 rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 max-w-4xl w-full overflow-hidden flex flex-col max-h-[90vh]">
            <div className="relative h-64 shrink-0">
              ...
              <div className="absolute top-6 left-6 flex gap-2">
                <span className="px-3 py-1.5 rounded-full bg-white/95 text-xs font-bold text-slate-800 shadow-sm border border-white/20">
                  {selectedCourse.category}
                </span>
                <span className="px-3 py-1.5 rounded-full bg-primary-700 text-xs font-bold text-white shadow-sm">
              ...
              <div className="absolute bottom-6 left-6 ">
                <p className="text-sm font-medium opacity-90 mb-1 tracking-wide flex items-center gap-2">
                  <MapPin size={14} weight="regular" className="text-white" />
            ...
            <div className="p-8 md:p-10 overflow-y-auto custom-scrollbar space-y-8">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedCourse.modules.map((item, idx) => (
                      <span
                        key={idx}
                        className="px-4 py-2 rounded-xl bg-primary-50 dark:bg-primary-900/20 text-primary-800 dark:text-primary-200 text-sm font-semibold border border-primary-100 dark:border-primary-800/50"
                      >
                        {item}
                      </span>
                    ))}
              ...
              {selectedCourse.opportunities &&
                selectedCourse.opportunities.length > 0 && (
